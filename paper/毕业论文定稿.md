...
在方法设计方面，本文构建了 GeoPFNMix，并进一步比较 GeoPFNMix-Lite、GeoPFNMix-CatBoost 及其 PFN 先验增强版本。结果显示，在当前比较范围内，`GeoPFNMix-CatBoost-TabPFN` 在主协议 `GroupKFold(CITY)` 下取得最优均值结果，即 `RMSE=5.9225`、`MAE=3.1078`、`R²=0.7622`；相较单模型 `CatBoost`，RMSE 下降 `1.47%`、MAE 下降 `3.09%`，属于幅度有限但方向稳定的边际提升，且 Wilcoxon 配对检验表明该改进具有统计显著性（`p=2.14e-21`）。同时，`GeoPFNMix-Lite-CatBoost-TabPFN` 取得 `RMSE=5.9281`、`MAE=3.1220`、`R²=0.7618`，成为当前第二优融合配置，并显著优于 `GeoPFNMix-Lite-CatBoost`（`p=1.17e-02`），说明在更强全局专家条件下，即使不启用残差分支，PFN 先验仍能提供稳定补充。
...
从表 14 可以看到，去除 `COUNTRY` 特征后所有模型的 RMSE 都明显上升，说明当前任务中来源代理信息确实对预测具有重要贡献。其中，单模型 `CatBoost` 的 RMSE 上升约 `5.92%`，而当前最佳模型 `GeoPFNMix-CatBoost-TabPFN` 的 RMSE 上升约 `3.89%`。这表明两点。第一，现有方法尚未摆脱对来源信息的依赖，因此不能把当前结果直接解读为“已经学到完全来源无关的普适环境规律”。第二，融合结构在一定程度上比单模型更能缓冲去除来源标签带来的性能损失，说明其收益并不完全等同于“简单记忆 COUNTRY”。结合表 13 和表 14，可以更稳妥地得出结论：GeoPFNMix 家族在当前多源任务上确实提供了稳定的结构性收益，但这种收益主要建立在“严格协议下的边际改进”和“相对更好的来源鲁棒性”之上，而非已经彻底解决跨来源外推问题。

综上，本章的实证结果可以归纳为三点。其一，严格地理协议显著抬高了任务难度，也使随机划分下的高分失去直接比较意义。其二，GeoPFNMix 家族在当前正式数据上确实优于强单模型基线，但主要体现为结构合理、方向稳定的边际增益，而非数量级的性能跃迁。其三，来源拆分、leave-one-country-out 和去除 `COUNTRY` 特征等补充分析共同说明：跨来源域偏移仍是决定模型上限的核心瓶颈，这也为下一章的讨论与结论奠定了边界条件。
...
另一方面，`GeoPFNMix-Lite-CatBoost-TabPFN` 已经非常接近当前冠军，而 `GeoPFNMix-Lite-CatBoost` 与 `GeoPFNMix-CatBoost` 之间也保持接近。这说明 CatBoost 骨干本身已经吸收了大部分可获得收益；残差分支和 PFN 先验带来的则是进一步的边际修正。由此得到的更稳妥判断不是“结构越复杂越好”，而是“在合适骨干之上，复杂度控制、层级校正和异构先验各自承担不同职责”。尤其是在 leave-one-country-out 验证中，最优结构转为 `GeoPFNMix-Lite-CatBoost`，进一步提示当域偏移显著增强时，更简洁的 CatBoost 家族结构反而可能比引入更多模块的复杂版本更稳健。
...
补充的鲁棒性验证进一步表明，真正的跨来源外推仍然是当前研究的主要难点。leave-one-country-out 下，所有模型的平均 `R²` 均为负，最佳均值结果转为 `GeoPFNMix-Lite-CatBoost`，其 `RMSE=9.5264`；同时，去除 `COUNTRY` 特征后，各模型性能均出现明显下降。这说明 GeoPFNMix 家族虽然已经能够在严格地理协议下提供稳定增益，但距离“来源无关的普适预测模型”仍有明显距离。
...
| GeoPFNMix 家族消融 | Lite、Full、CatBoost、TabPFN 等变体系统比较 | `GeoPFNMix-CatBoost-TabPFN` 在主协议下达到 `RMSE=5.9225` | 结构融合有效，但主要体现为强基线之上的边际稳健增益 |
...
总体而言，本文的价值不只在于给出了一个最终可复现的最优模型，更在于建立了一条完整、连贯且可审计的研究证据链：数据标准化保证了不同来源样本可以进入同一建模语境；严格分组协议保证了结论不会被地理泄漏高估；强基线体系保证了创新方法的比较对象足够充分；消融、显著性检验、来源拆分分析、跨来源外推验证与 SHAP 解释则共同说明了性能改进来自哪里，又在哪些场景下仍然存在局限。也正因为如此，本文更稳妥的结论应当是“GeoPFNMix 家族在当前设置下具有研究价值且能够提供边际稳健增益”，而不是简单宣称某一复杂结构已经在所有意义上完成了决定性超越。
