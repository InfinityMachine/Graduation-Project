...
可见，在严格地理分组协议下，Boosting 树整体竞争力更强，CatBoost 相比单模型 TabPFN 的 RMSE 进一步降低约 `9.35%`。这说明在更强的跨域异质性条件下，显式利用 `COUNTRY` 与分层类别信息的能力，比单纯依赖小样本先验更重要。
...
需要说明的是，表 5 与表 6 中同名 `CatBoost` 的均值结果存在极小差异，例如主协议下分别为 `6.0109` 与 `6.0138`。这一差异来自 `run_baselines.py` 与 `run_geopfnmix.py` 两个独立脚本批次的正式重跑；在 GPU 环境下，即便随机种子固定，独立批次运行仍可能产生 `0.01` 量级以内的微小波动。由于这一差异远小于不同模型之间的性能差距，因此不会改变本文的任何结论。
...
上述结果最值得注意的现象是 `COUNTRY` 的重要性显著高于其他特征。这说明在多源数据场景下，跨国家或跨区域来源差异已经成为模型必须首先处理的域偏移因素。与此同时，`SO2`、`NOX`、`NOX_div_SO2`、`NOX_plus_SO2` 等污染前体物及其协同特征，与 `AET`、`SO2_x_wind` 等环境调制变量共同构成了第二层关键影响因素，而 `CITY` 仍提供了有限但不可忽视的局部地理信息。该解释结果与前述协议差异和误差结构分析相互印证，说明本文的特征设计和方法结构与问题本身具有较好一致性。
...
首先，多源数据显著放大了类别层级的重要性。SHAP 结果显示 `COUNTRY` 是全局专家最重要的特征，而 `CITY` 仍位于前十特征之中，同时污染前体物与交互项占据了大部分高位。这意味着跨国家来源差异与局部地理结构共同塑造了建模难度。在这种条件下，CatBoost 对类别特征与复杂非线性的联合建模优势，直接转化为更强的全局骨干能力。
